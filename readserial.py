...
async def create_serial_connection(loop, protocol_factory, communication,
                                   *args, **kwargs):
    ser = serial.Serial(*args, **kwargs)
    protocol = protocol_factory()
    protocol.queue = communication
...
class Output(asyncio.Protocol):
    ...
    def frame_received(self, data):
        try:
            ...
            if out[0]:
                print("Nic: {}".format(out[1]/1000000))
                self.queue.put_nowait("Nic: {}".format(out[1]/1000000))
            else:
                print("Tom: {}".format(out[1]/1000000))
                self.queue.put_nowait("Tom: {}".format(out[1]/1000000))
            return True
    ...
    def data_received(self, data):
        ...
        index = self.headre.search(alldata)
        if index:
        ...
        while len(alldata) >= 7:
            ...
            framebuilt = self.frame_received(packet)
            if not framebuilt:
                index = self.headre.search(alldata, currentoffset +1,
                                            messageEnd)
                if index:
